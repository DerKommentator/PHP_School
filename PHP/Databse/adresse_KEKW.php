...
if(isset($_GET['edit_id']))
{
    ...
    echo '<div style="background-color:#5fefef;padding:3px;"><input type="text" name="vorname" value="'.$_GET['vorname'].'"></div>';
    echo '<div style="background-color:lightgray;padding:3px;">Nachname</div>';
    echo '<div style="background-color:#5fefef;padding:3px;"><input type="text" name="nachname" value="'.$_GET['nachname'].'"></div>';
    ...
    echo '</div>';
    echo '<input type="hidden" name="adresse_id" value="'.$_GET['id'].'">';
    echo '</form>';
    echo '</div>';
}
...
{
    echo 'Soll der Datensatz '.$_GET['vorname'].' '.$_GET['nachname'].' wirklich gelöscht werden?<br>';
...
if (!$_GET or isset($_GET['Zurück']) or isset($_GET['edit_save']) or isset($_GET['insert_save']) or isset($_GET['delete']))
{
    ...
    while ($row=$result->fetch_assoc())
    {
        ...
        echo '<td><button type="submit" name="edit_id"><img src="icons/aendern.gif" width="30px" alt="Submit"></button></td>';
        echo '<td><button type="submit" name="show_id"><img src="icons/lupe.bmp" width="30px" alt="Submit"></button></td>';
        echo '<td><button type="submit" name="delete_id"><img src="icons/delete.bmp" width="30px" alt="Submit"></td>';
        echo '</tr>';

        echo '<input type="hidden" name="id" value="'.$row['adresse_id'].'">';
        echo '<input type="hidden" name="vorname" value="'.$row['vorname'].'">';
